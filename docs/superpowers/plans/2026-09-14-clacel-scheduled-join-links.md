...
**Global Constraints:** Clacelのみ対象。既存の`mode=join&room=...`、TOEIC、IELTS、通常の採点・復習ロジックは変えない。URLの改変・対象期間外・未来日・終了済み・開始後の新規参加を拒否する。秘密値はリポジトリへ保存しない。
...
3. 開始後・終了後・未来日・期限切れ・不正リンクの案内を表示する失敗テストを書く。
